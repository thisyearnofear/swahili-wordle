...
const createSetState = <T extends keyof AppState>(property: T) => {
  return (state: AppState, action: PayloadAction<AppState[T]>) => {
    state[property] = action.payload;
  };
};
const getRandomWord = (words: string[]) => encode(words[Math.floor(Math.random() * words.length)]);
